...
/* Based on lstrip code by Luiz Henrique de Figueiredo */
/* BEGIN */

static void lbaux_quote(lua_State *L,const TString *ts) {
  size_t i; luaL_Buffer b;
  size_t n = ts->tsv.len; 
...
#define lbaux_pair(a,b) (1024*(int)(a)+(int)(b))
...
static int lbaux_toustr32(lua_State *L) {
  ...
  char buf[4]; /* Flawfinder: ignore */
  memcpy(buf,&n,4); /* Flawfinder: ignore */
...
static int lbaux_touint32(lua_State *L) {
  ...
  if (size == 4) {
    uint32_t n;
    memcpy(&n,buf,4); /* Flawfinder: ignore */
    lua_pushnumber(L,n);
  } else
    lua_pushnil(L);
  return 1;
...
static int lbaux_fchmod(lua_State *L) {
